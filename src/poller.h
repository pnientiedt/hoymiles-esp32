...
// discovered PV port numbers (bounded by the pv_data proto array, currently 4)
// so the caller can zero per-panel energy_today while the inverter is offline.
// The caller's buffer must hold that many (MAX_PV_PORTS). Untouched on failure
// or when NULL.
